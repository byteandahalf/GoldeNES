...
typedef uint16_t address; // The 6502 has a 16-bit address bus, so addresses are only 2 bytes
typedef byte reg;
...
struct emu6502 {
	...
	struct RAM {
		...
		static short MAX_ADDRESS = 0xFFFF;
		static byte ZERO_PAGE_LEN = 0xFF; // Zero Page is 0x00 - 0xFF
